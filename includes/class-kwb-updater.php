...
final class KWB_GitHub_Updater {
	...
	const SLUG='woocommerce-workshop-bookings';
	...
	public static function maybe_force_refresh(){
		...
		global $pagenow;
		if('update-core.php'===$pagenow&&!empty($_GET['force-check']))delete_site_transient('kwb_github_release');
	}
	...
	private static function release(){
		$c=get_site_transient('kwb_github_release');
		if(is_array($c)&&!empty($c['version'])&&!empty($c['fetched_at'])){
			$age=time()-absint($c['fetched_at']);
			if(version_compare((string)$c['version'],self::VERSION,'>')||$age<10*MINUTE_IN_SECONDS)return$c;
		}
		$r=wp_remote_get(self::API,array('timeout'=>12,'redirection'=>0,'headers'=>array('Accept'=>'application/vnd.github+json','User-Agent'=>'WooCommerce-Workshop-Bookings/'.self::VERSION)));
		...
		$z='woocommerce-workshop-bookings.zip';$h=$z.'.sha256';$s=$z.'.sig';
		if(empty($a[$z])||empty($a[$h])||empty($a[$s])||!self::trusted($a[$z],$z)||!self::trusted($a[$h],$h)||!self::trusted($a[$s],$s))return array();
		$hr=wp_remote_get($a[$h],array('timeout'=>10,'redirection'=>5,'headers'=>array('User-Agent'=>'WooCommerce-Workshop-Bookings/'.self::VERSION)));if(is_wp_error($hr)||200!==wp_remote_retrieve_response_code($hr)||!preg_match('/\b([a-f0-9]{64})\b/i',wp_remote_retrieve_body($hr),$m))return array();
		$d=array('version'=>$v,'package'=>$a[$z],'sha256'=>strtolower($m[1]),'signature'=>$a[$s],'url'=>esc_url_raw((string)($j['html_url']??'')),'body'=>wp_kses_post((string)($j['body']??'')),'fetched_at'=>time());set_site_transient('kwb_github_release',$d,10*MINUTE_IN_SECONDS);return$d;
	}
	...
	public static function verify_download($reply,$package,$upgrader,$hook_extra){
		...
		$sr=wp_remote_get($r['signature'],array('timeout'=>15,'redirection'=>5,'headers'=>array('User-Agent'=>'WooCommerce-Workshop-Bookings/'.self::VERSION)));if(is_wp_error($sr)||200!==wp_remote_retrieve_response_code($sr)){@unlink($tmp);return new WP_Error('kwb_signature_missing','Update blocked: digital signature unavailable.');}
