...
final class KWB_Booking {
	...
	public static function panel(){
		...
		<?php
		woocommerce_wp_checkbox(array('id'=>self::META_ENABLED,'label'=>__('Ενεργοποίηση booking','kangiroo-workshop-bookings'),'value'=>$g(self::META_ENABLED)));
		woocommerce_wp_checkbox(array('id'=>self::META_MONTHLY,'label'=>__('Μηνιαία συμμετοχή','kangiroo-workshop-bookings'),'description'=>__('Δεσμεύει θέση σε όλες τις συναντήσεις του επιλεγμένου μήνα.','kangiroo-workshop-bookings'),'value'=>$g(self::META_MONTHLY)));
		woocommerce_wp_text_input(array('id'=>self::META_MONTHLY_PRICE,'label'=>__('Τιμή / μήνα (€)','kangiroo-workshop-bookings'),'value'=>$g(self::META_MONTHLY_PRICE),'type'=>'number','custom_attributes'=>array('min'=>'0','step'=>'0.01')));
		woocommerce_wp_checkbox(array('id'=>self::META_SINGLE,'label'=>__('Μεμονωμένη συμμετοχή','kangiroo-workshop-bookings'),'value'=>$g(self::META_SINGLE)));
		...
		woocommerce_wp_text_input(array('id'=>self::META_HORIZON,'label'=>__('Μήνες διαθέσιμοι μπροστά','kangiroo-workshop-bookings'),'value'=>$g(self::META_HORIZON,KWB_Settings::get('default_horizon_months',3)),'type'=>'number','custom_attributes'=>array('min'=>'1','max'=>'12','step'=>'1')));
		woocommerce_wp_text_input(array('id'=>self::META_REMINDER_OVERRIDE,'label'=>__('Reminder override (λεπτά)','kangiroo-workshop-bookings'),'description'=>__('Κενό = γενική ρύθμιση.','kangiroo-workshop-bookings'),'value'=>$g(self::META_REMINDER_OVERRIDE),'type'=>'number','custom_attributes'=>array('min'=>'5','max'=>'10080','step'=>'1')));
		woocommerce_wp_select(array('id'=>self::META_RSVP_OVERRIDE,'label'=>__('RSVP για αυτό το εργαστήριο','kangiroo-workshop-bookings'),'value'=>$g(self::META_RSVP_OVERRIDE,'inherit'),'options'=>array('inherit'=>'Κληρονόμηση γενικής ρύθμισης','on'=>'Ενεργό','off'=>'Ανενεργό')));
		woocommerce_wp_text_input(array('id'=>self::META_BOOKING_CUTOFF,'label'=>__('Κλείσιμο κρατήσεων πριν','kangiroo-workshop-bookings'),'description'=>__('Λεπτά πριν την έναρξη. Κενό = γενική/χωρίς περιορισμό.','kangiroo-workshop-bookings'),'value'=>$g(self::META_BOOKING_CUTOFF),'type'=>'number','custom_attributes'=>array('min'=>'0','max'=>'10080','step'=>'1')));
		woocommerce_wp_text_input(array('id'=>self::META_MAX_QTY,'label'=>__('Μέγιστα παιδιά ανά κράτηση','kangiroo-workshop-bookings'),'description'=>__('Κενό = χωρίς ειδικό όριο.','kangiroo-workshop-bookings'),'value'=>$g(self::META_MAX_QTY),'type'=>'number','custom_attributes'=>array('min'=>'1','max'=>'100','step'=>'1')));
		woocommerce_wp_text_input(array('id'=>self::META_LOCATION,'label'=>__('Τοποθεσία override','kangiroo-workshop-bookings'),'description'=>__('Κενό = γενική τοποθεσία.','kangiroo-workshop-bookings'),'value'=>$g(self::META_LOCATION)));
		?>
